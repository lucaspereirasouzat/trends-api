...
export class DailyTrendsUseCase {
  ...
  async execute(request: DailyTrendsRequest): Promise<{ data: unknown } | { error: string }> {
    ...
    const data = await this.gateway.dailyTrends(options as Parameters<GoogleTrendsGateway["dailyTrends"]>[0]);
    return { data };
  }
}
